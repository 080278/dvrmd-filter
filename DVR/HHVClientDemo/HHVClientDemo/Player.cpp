...
INT CPlayer::StartMonitor(HWND hWnd, HHV_CLIENT_INFO* clientInfo)
{
    ...
    if( ret < 0 )
    {
        //TRACE_LOG("(MTVideo)监视失败 发送cmd失败或被服务器拒绝 vsIP = %s vsPort = %d dvrIP = %s channel = %d\r\n",
		//	clientInfo->RFIP, clientInfo->RFTCPPort, clientInfo->connInfo.ip, clientInfo->channel);
        m_comSocket.Close();
        return ret;
    }
    //TRACE_LOG("(MTVideo)监视 发送cmd成功 vsIP = %s vsPort = %d dvrIP = %s channel = %d\r\n",
	//	clientInfo->RFIP, clientInfo->RFTCPPort, clientInfo->connInfo.ip, clientInfo->channel);

    //mode- 流模式（1-实时流/2-文件流）
	int nErr = 0;
	m_playType = STREAME_REALTIME;
	BOOL  bret = HK_PlayM4_SetStreamOpenMode(m_index, m_playType);
    bret = HK_PlayM4_OpenStream( m_index, (BYTE*)m_streamHeader, streamHeaderSize, 600*1024);
	if (!bret)
	{
		m_comSocket.Close();
		nErr = HK_PlayM4_GetLastError(m_index);
		//string strErr = GetPlayMp4LastErrorStr( nErr );
		//TRACE_SDK_ERROR("执行 CHKSeries::OpenStream 中 HK_PlayM4_OpenStream 函数 Fail nErr = %d strErr = %s\r\n\r\n", nErr, strErr.c_str() );
	...
	if( !HK_PlayM4_RigisterDrawFun( m_index, MP4SDKDrawFun, (DWORD)this) )
	{
		m_comSocket.Close();
		nErr = HK_PlayM4_GetLastError(m_index);
		//TRACE_SDK_ERROR("执行 CHKSeries::RegisterDrawFun 中 HK_PlayM4_RigisterDrawFun 函数 Fail nErr = %d strErr = %s\r\n\r\n", nErr, strErr.c_str() );
		return -nErr;
	}

	bret = HK_PlayM4_Play(m_index, hWnd);
	if (!bret)
	{
		nErr = HK_PlayM4_GetLastError(m_index);
		//string strErr = GetPlayMp4LastErrorStr( nErr );
		//TRACE_SDK_ERROR("执行 CHKSeries::OpenStream 中 HK_PlayM4_Play 函数 Fail nErr = %d strErr = %s\r\n\r\n", nErr, strErr.c_str() );
		return -nErr;
	}

    TRACE_LOG("(MTVideo)监视 OpenPlayStream成功 m_index = %d dvrIP = %s channel = %d\r\n",
    ...
    if(m_Thread <= 0)
    {
        TRACE_LOG("(MTVideo)!!!!!!开辟接受数据线程失败！m_Thread:%d errno:%d _doserrno:%d \r\n",
    ...
    TRACE_LOG("(MTVideo)监视成功结束 m_PlayHandle = %d dvrIP = %s channel = %d\r\n",
		m_PlayHandle, clientInfo->connInfo.ip, clientInfo->channel);
    return m_index;
}
VOID CPlayer::StopMonitor( )
{
	TRACE_LOG("(MTVideo)停止监视 开始\r\n");
    m_exit = true;
    m_comSocket.Close();
    //TRACE_LOG("(MTVideo)停止监视 将执行WaitForSingleObject dvrIP = %s channel = %d m_PlayHandle = %d m_index_MTManager = %d\r\n",
    ...
    {
        TRACE_LOG("(MTVideo)!!!!!停止监视时 WaitFor退出信号超时 \r\n");
    }
    
	BOOL bret = HK_PlayM4_Stop(m_index);
	...
	if (!bret)
	{
		nErr = HK_PlayM4_GetLastError(m_index);
		//string strErr = GetPlayMp4LastErrorStr( nErr );
		//TRACE_SDK_ERROR("执行 CHKSeries::CloseStream 中 HK_PlayM4_Stop 函数 Fail nErr = %d strErr = %s\r\n\r\n", nErr, strErr.c_str() );
		//return -nErr;
	}
	
	
	bret = HK_PlayM4_CloseStream( m_index);
	if (!bret)
	{
		nErr = HK_PlayM4_GetLastError(m_index);
		//string strErr = GetPlayMp4LastErrorStr( nErr );
		//TRACE_SDK_ERROR("执行 CHKSeries::CloseStream 中 HK_PlayM4_CloseStream 函数 Fail nErr = %d strErr = %s\r\n\r\n", nErr, strErr.c_str() );
		//return -nErr;
	}
	
	
    //TRACE_LOG("(MTVideo)停止监视成功 DvrIP = %s channnel = %d m_PlayHandle = %d m_index_MTManager = %d\r\n",
	//	m_UniSDKCltInfo.connInfo.ip, m_UniSDKCltInfo.channel, m_PlayHandle, m_index_MTManager);
    m_PlayHandle = -1;
}
...
int CPlayer::InputData()
{
    ...
    if( ret <= 0 )
    {
        if(m_exit)
        {
            //TRACE_LOG("CMTVideo::InputData 退出1 dvrIP = %s Channel = %d m_PlayHandle = %d m_index_MTManager = %d ret = %d nReceiveOneTime = %dkbps\r\n",
			//	m_UniSDKCltInfo.connInfo.ip, m_UniSDKCltInfo.channel, m_PlayHandle, m_index_MTManager, ret, nReceiveOneTime);
            return -1;
        }
        //TRACE_LOG("CMTVideo::InputData <m_comSocket.Recv Fail> dvrIP = %s Channel = %d m_PlayHandle = %d  m_index_MTManager = %d ret = %d nReceiveOneTime = %dkbps\r\n\r\n",
		//	m_UniSDKCltInfo.connInfo.ip, m_UniSDKCltInfo.channel, m_PlayHandle, m_index_MTManager, ret, nReceiveOneTime);
        //TRACE_LOG("！！！！检测到网络出错，准备发送消息 majortype = %d, subtype = %d, m_index_MTManager = %d\r\n\r\n",
    ...
    if(m_exit)
    {
        //TRACE_LOG("CMTVideo::InputData 退出2 dvrIP = %s Channel = %d m_PlayHandle = %d m_index_MTManager = %d nReceiveOneTime = %dkbps\r\n",
			//m_UniSDKCltInfo.connInfo.ip, m_UniSDKCltInfo.channel, m_PlayHandle, m_index_MTManager, nReceiveOneTime);
        return -1;
    }

	BOOL bret = HK_PlayM4_InputData(m_index, (BYTE*)m_buffer, ret);
	if (!bret)
	{
		int nErr = HK_PlayM4_GetLastError(m_index);
		if( nErr == PLAYM4_BUF_OVER )
			return 0;
		
		//string strErr = GetPlayMp4LastErrorStr( nErr );
		//TRACE_SDK_ERROR("执行 CHKSeries::SetStreamBuf 中 HK_PlayM4_InputData 函数 Fail nErr = %d strErr = %s\r\n\r\n", nErr, strErr.c_str() );
		
		//TRACE_LOG("CMTVideo::InputData 退出3 dvrIP = %s Channel = %d m_PlayHandle = %d m_index_MTManager = %d nReceiveOneTime = %dkbps\r\n\r\n",
		//	m_UniSDKCltInfo.connInfo.ip, m_UniSDKCltInfo.channel, m_PlayHandle, m_index_MTManager, nReceiveOneTime);
        //TRACE_LOG("！！！！inputdata<0检测到网络出错，准备发送消息 majortype = %d, subtype = %d, m_index_MTManager = %d\r\n\r\n",
		//	m_UniSDKCltInfo.connInfo.dvrType, m_UniSDKCltInfo.connInfo.subType, m_index_MTManager);
        //::PostMessage(m_hNotifyWnd, WM_NET_ERROR, m_index_MTManager, m_UniSDKCltInfo.connInfo.dvrType << 16 | m_UniSDKCltInfo.connInfo.subType );
...
int CPlayer::InputData_Frame()
{
    ...
    if( ret <= 0 )
    {
        if(m_exit)
        {
            //TRACE_LOG("CMTVideo::InputData_Frame 退出1 dvrIP = %s Channel = %d m_PlayHandle = %d m_index_MTManager = %d\r\n",
			//	m_UniSDKCltInfo.connInfo.ip, m_UniSDKCltInfo.channel, m_PlayHandle, m_index_MTManager);
            return -1;
        }
        //TRACE_LOG("CMTVideo::InputData_Frame <m_streamParser.GetOneFrame Fail> dvrIP = %s Channel = %d m_PlayHandle = %d  m_index_MTManager = %d\r\n\r\n",
		//	m_UniSDKCltInfo.connInfo.ip, m_UniSDKCltInfo.channel, m_PlayHandle, m_index_MTManager);
        //TRACE_LOG("！！！！检测到网络出错，准备发送消息 majortype = %d, subtype = %d, m_index_MTManager = %d\r\n\r\n",
		//	m_UniSDKCltInfo.connInfo.dvrType, m_UniSDKCltInfo.connInfo.subType, m_index_MTManager);
        //::PostMessage(m_hNotifyWnd, WM_NET_ERROR, m_index_MTManager, m_UniSDKCltInfo.connInfo.dvrType << 16 | m_UniSDKCltInfo.connInfo.subType );
    ...
    if(m_exit)
    {
        //TRACE_LOG("CMTVideo::InputData_Frame 退出2 dvrIP = %s Channel = %d m_PlayHandle = %d m_index_MTManager = %d\r\n",
		//	m_UniSDKCltInfo.connInfo.ip, m_UniSDKCltInfo.channel, m_PlayHandle, m_index_MTManager);
        return -1;
    }
	int encFrameLength = ret - m_frameHeader.MetaLength;
    BOOL bret = HK_PlayM4_InputData(m_index, (BYTE*)m_buffer, encFrameLength);
	if (!bret)
	{
		int nErr = HK_PlayM4_GetLastError(m_index);
		if( nErr == PLAYM4_BUF_OVER )
...
{    
    ...
    {
        m_exit = true;
        TRACE_LOG("(存储回放 解包1)error: GetOneFrame<0 ret=%d\r\n", nret);
        if( nret == READ_DATA_OVER )//读数据结束
        {
            //等待播放结束
			int playRemain = HK_PlayM4_GetSourceBufferRemain(m_index);
			while (playRemain > 0)
			{
				Sleep(8000);
				playRemain = HK_PlayM4_GetSourceBufferRemain(m_index);
			}
			return 0;
        }
        return -1;
	...
	while ( nLeft > 0 )
	{
		...
		BOOL bret = HK_PlayM4_InputData(m_index, (BYTE*)pDst, nLeft);
        if (!bret)
		{
			int nErr = HK_PlayM4_GetLastError(m_index);
			...
			{
				Sleep(100);
				bret = HK_PlayM4_InputData(m_index, (BYTE*)pDst, nLeft);
				if (!bret)
				{
					nErr = HK_PlayM4_GetLastError(m_index);
				}
...
{   
	...
	{
        TRACE_LOG("(MonitorStartCmdMT) 发送头失败\r\n");
	...
	{
        //TRACE_LOG("(MonitorStartCmdMT) 发送nRouteInfo失败\r\n");
        //CCommClass::ErrorDeal(pErrorMsg, CLIENT_ERROR, VCT_NETERROR_SEND, false);
    	return HHV_ERROR_SEND;
    }
    ...
    if(nRouteInfo > 0)
    {
		...
		{
            //TRACE_LOG("(MonitorStartCmdMT) 发送pRouteInfo失败\r\n");
        	//CCommClass::ErrorDeal(pErrorMsg, CLIENT_ERROR, VCT_NETERROR_SEND, false);
    		return HHV_ERROR_SEND;
    	}
    }
	...
	{
        TRACE_LOG("(MonitorStartCmdMT) Accept:%d n:%d\r\n",
            response.header.command, htonl(response.header.command));
        TRACE_LOG("(MonitorStartCmdMT)回应命令ACCEPT streamheadsize = %d dvrIP = %s channel=%d\r\n",
		...
		{
            TRACE_LOG("(MonitorStartCmdMT) 收streamHeader失败！streamheadsize = %d dvrIP = %s\r\n",
                    response.streamheadsize, clientInfo->connInfo.ip);
            //CCommClass::ErrorDeal(pErrorMsg, CLIENT_ERROR, VCT_NETERROR_RECV, false);
            return HHV_ERROR_RECV;
        }
        size = response.streamheadsize;
        TRACE_LOG("(MonitorStartCmdMT)成功结束 dvrIP = %s channel=%d\r\n",
    ...
    else
    {
        ...
        TRACE_LOG("(MonitorStartCmdMT)服务器拒绝 cmd=%d(%d) reason=%d errtype(sdk(0) server(1))=%d dvrIP=%s channel=%d\r\n",
